...
//  gfile(FILE* __fp);    // Construct from ANSI stream pointer
    ...
    FILE*   Popen   (const std::string& __path, const char* __mode)
    {
        return Popen(__path.c_str(), __mode);
    }
    int     Pclose  ();
    ...
    {
        Fread(&i, sizeof(uint8_t));
        return *this;
    }
    gfile& operator>> (uint16_t& i)
    {
        Fread(&i, sizeof(uint16_t));
        return *this;
    }
...
//  int     Printf  (const char* __format, ...) __attribute__ ((format (printf, 2, 3)));
...
#ifdef __GOLDWARE_HAS_BOOL
    gnfile& operator>> (bool& i)
    {
        if( sizeof(bool) ==1 ) Fread(&i, 1);
        else for( size_t c=0; c<sizeof(bool) )
            {
                ((char*)i)[c] = Fgetc();
            };
        return *this;
...
#endif
    gnfile& operator>> (uint8_t& i)
    {
        Fread(&i, sizeof(uint8_t));
        return *this;
    }
    gnfile& operator>> (uint16_t& i)
    {
        char c[2];
        Fread(c, 2);
        i = uint16_t(c[0])
    ...
    */
    ...
    gnfile& operator<< (uint32_t o)
    {
        ...
        o8 = uint8_t((o<<8) & 0xff);
        Fwrite(&o8, sizeof(o8));
        o8 = uint8_t((o<<16) & 0xff);
        Fwrite(&o8, sizeof(o8));
        o8 = uint8_t((o<<24) & 0xff);
        Fwrite(&o8, sizeof(o8));
        return *this;
    }
    /*
