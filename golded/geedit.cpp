...
UndoStack::~UndoStack()
{
    ...
    while(last_item)
    {
        ...
        case EDIT_UNDO_DEL_LINE:
            delete(last_item->data.line_ptr);
        }
        delete last_item;
    }
}
...
void UndoStack::PushItem(uint action, Line* __line, uint __col, uint __len)
{
    ...
    if(undo_enabled)
    {
        ...
        case EDIT_UNDO_DEL_TEXT:
            ...
            last_item->data.text_ptr = new(__len) text_item(__col, __len);
        ...
        case EDIT_UNDO_WRAP_TEXT:
            ...
            if(__len == NO_VALUE)
                __len = __line->txt.length() - __col;
            (last_item->data.text_ptr = new text_item(__col, __len));
            break;
