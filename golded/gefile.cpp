...
void FileSelect(GMsg* msg, char* title, FileSpec* fspec)
{
    ...
    while((de = f.nextentry(wildlistname)) != NULL)
    {
        if(not de->is_file())
            continue;
        FFblk* fb;
        fblk = (FFblk*)realloc(fblk, (totfiles+1)*sizeof(FFblk));
        fb = fblk + totfiles;
    ...
    if(totfiles)
    {
        ...
        else
        {
            ...
            fspec->fblk = fblk = (FFblk*)realloc(fblk, (selfiles+1)*sizeof(FFblk));
...
void CreateFileAddr(GMsg* msg)
{
    ...
    msg->txt = (char*)realloc(msg->txt, _txt.length()+1);
...
void CreateFileMsgs(int mode, GMsg* msg)
{
    ...
    *buf = NUL;
    ptr = strdup(msg->txt);   // Keep a copy of the original
