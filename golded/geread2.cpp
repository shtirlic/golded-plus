...
void GotoMsgno()
{
    ...
    if(AA->Msgn.Count())
    {
        ...
        uint32_t* replies = (uint32_t*)calloc(list_max+1, sizeof(uint32_t));
...
int ExternUtil(GMsg *msg, const ExtUtil &extutil)
{
    ...
    if(extutil.options & EXTUTIL_RELOAD)
    {
        ...
        if(not strstr(msg->txt, "\001CHRS:") and not strstr(msg->txt, "\001CHARSET:"))
        {
            ...
            char *msg_txt_chrs = (char *)malloc(strlen(msg->txt)+strlen(charset)+sizeof("\001CHRS:  2\r")+16);
        ...
        if(AA->Viewkludge())
        {
            ...
            {
                // do recode
                char *msg_txt_chrs = (char *)malloc(strlen(msg->txt)*3+16);
                XlatStr(msg_txt_chrs, msg->txt, level, CharTable);
                free(msg->txt);
                msg->txt = (char *)realloc(msg_txt_chrs, strlen(msg_txt_chrs)+16);
            }
        }
    ...
    for (int n = tline; n < msg->lines; n++)
    {
        ...
        while (*ptr)
        {
            ...
            if (urlBegin and not strneql(ptr, "mailto:", 7))
            {
                ...
                if (urlBegin < urlEnd)
                {
                    char* bufurl=(char*)malloc(urlEnd - ptr + 3);
                    bufurl[0] = ' ';
...
void Make_Userlist(const char* userlist)
{
    ...
    crclist = (word*)calloc(AA->Msgn.Count()+1, sizeof(word));
