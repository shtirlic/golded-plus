...
void Edit__killpastebuf()
{
    ...
    while(Edit__pastebuf)
    {
        if(Edit__pastebuf->next)
        {
            ...
            delete(Edit__pastebuf->prev);
        }
        else
            delete(Edit__pastebuf);
    }
}
...
void Edit__killkillbuf()
{
    ...
    while(Edit__killbuf)
    {
        if(Edit__killbuf->prev)
        {
            ...
            delete(Edit__killbuf->next);
        }
        else
            delete(Edit__killbuf);
    }
}
...
void IEclass::LoadFile()
{
    ...
    if (_fp.isopen())
    {
        ...
        msgptr->txt = (char*)calloc(1, (uint)(_fp.FileLength()+256));
...
#define binary     (uuencode or base64)
    ...
    if (fileselected or getclip)
    {
                ...
                (fp.Fopen(filenamebuf.c_str(), binary ? "rb" : "rt", CFG->sharemode))!=NULL)
        {
            ...
            char* _parabuf = (char*)malloc(EDIT_PARABUFLEN);
            ...
            else
            {
                ...
                char* buf = (char*) malloc(EDIT_PARABUFLEN);
