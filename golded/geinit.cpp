...
static void ReadEcholists()
{
    ...
    Echo* echoin = (Echo*)calloc(1, sizeof(Echo));
    ...
    if (fp.isopen())
    {
        ...
        while (fp.Fgets(buf, sizeof(buf)))
        {
            ...
            {
                echonums++;
                echoin = (Echo*)realloc(echoin, (echonums+2)*sizeof(Echo));
...
static void ReadEscsets()
{
    ...
    for (int n = 0; x != CFG->xlatescset.end(); x++, n++)
    {
        ...
        if (fp.isopen())
        {
            ...
            {
                CompTable = (Esc*)realloc(CompTable, sizeof(Esc));
                fp.Fread(CompTable, sizeof(Esc));
                CompTP = CompTable->t;
            }
            else if (strieql(x->imp, "I51"))
            {
                I51Table = (Esc*)realloc(I51Table, sizeof(Esc));
                fp.Fread(I51Table, sizeof(Esc));
                I51TP = I51Table->t;
            }
            else if (strieql(x->imp, "MNEMONIC"))
            {
                MNETable = (Esc*)realloc(MNETable, sizeof(Esc));
