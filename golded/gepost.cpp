...
void CheckSubject(GMsg* msg, char* subj)
{
    ...
    for(int n=0; n<fspecs; n++)
        free(fspec[n].fblk);
    fspecs = specfiles = 0;
    fspec = (FileSpec*)realloc(fspec, sizeof(FileSpec));
        ...
        if(msg->attr.frq())
        {
            ...
            if(*subj)
            {
                ...
                while(ptr)
                {
                    ...
                    {
                        fspec = (FileSpec*)realloc(fspec, (fspecs+1)*sizeof(FileSpec));
    ...
    if(AL.AreaEchoToNo(AA->Areayouwroteto())>=0)
    {
        ...
        if(AA != A)
        {
            ...
            if(not strnieql(ptr, "AREA:", 5))
            {
                ...
                msg->txt = (char*)realloc(msg->txt, elen+mlen);
                back = msg->txt+elen;
...
static void GetLastLink(GMsg* msg, uint32_t& msgno)
{
    ...
    GMsg* uplink = (GMsg*)calloc(1, sizeof(GMsg));
...
void MakeMsg(int mode, GMsg* omsg, bool ignore_replyto)
{
    ...
    // Allocate some msgs
    GMsg* msg = (GMsg*)calloc(1, sizeof(GMsg));
    GMsg* reply = (GMsg*)calloc(1, sizeof(GMsg));
    GMsg* cmpmsg = (GMsg*)calloc(1, sizeof(GMsg));
        ...
        if(mode != MODE_COPY)
        {
            ...
            case MODE_REPLY:
                ...
                if(AA->isnet())
                {
                    ...
                    {
                        free(msg->inreplyto);
                        msg->inreplyto = strdup(omsg->messageid);
                    }
                ...
                else
                {
                    ...
                    msg->references = (char*)calloc(1, reflen+1+midlen+1);
                    if(omsg->references)
            ...
            if(status == NO)
            {
                ...
                else
                {
                    if (confirm)
                    {
                        ...
                        if (msg->txt == NULL)
                            msg->txt = strdup("\r\rConfirmation Receipt\r\r");
