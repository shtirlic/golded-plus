...
void TokenXlat(int mode, char *&input, size_t size, bool resize, GMsg* msg, GMsg* oldmsg, int origarea)
{
    ...
    if (resize)
    {
        ...
        if (size < newsize)
            input = (char*)realloc(input, size = newsize);
    }
...
#define PBUFSIZE 4096   // Allow a 4k long paragraph
        ...
        msg->txt = txtptr = (char*)realloc(msg->txt, tlen);
...
void GMsg::LinesToText()
{
    ...
    // Allocate the text
    txt = (char*)realloc(txt, _alloc_size);
        ...
        if(_line->isheader())
        {
            ...
            while(_line->next and _line->iswrapped())
            {
                ...
                if((_bptr-_buf) > (bufsize-256))
                {
                    ...
                    bufsize += 4096;
                    _buf = (char*)realloc(_buf, bufsize);
                    _bptr = _buf + bptrlen;
                }
            }
        }
