...
void NodelistBrowser::Open()
{
    ...
    entries = (ftn_nodelist_entry*)calloc(maxpos, sizeof(ftn_nodelist_entry));
    liststr = (ListStr*)calloc(maxpos, sizeof(ListStr));
...
void LookupNodeLocation(GMsg* msg, std::string &location, int what)
{
    ...
    else
    {
        ...
        if (strbag.First())
        {
            char *city_upr = (char*)malloc(city.length()+1);
            ...
            do
            {
                ...
                if (ptr)
                {
                    ...
                    city_upr = (char*)realloc(city_upr, city.length()+1);
                    strcpy(city_upr, city.c_str());
                    strupr(city_upr);
                }
            }
...
void CheckNodelists()
{
    ...
    if(ft != oldft)
    {
        ...
        if (fp.isopen())
        {
            ...
            {
                NODE->nodelist = (Stamp*)realloc(NODE->nodelist, (NODE->nodelists+1)*sizeof(Stamp));
                char* key;
                char* val=buf;
                getkeyval(&key, &val);
